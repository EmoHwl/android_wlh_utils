...
import com.tool.phoneutils.utils.FileDownloader;
import com.tool.phoneutils.utils.FileService;
...
public class DownloadActivity extends Activity {
    ...
    private void download(final String url, final File savedir) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                FileDownloader loader = new FileDownloader(DownloadActivity.this, url, savedir, 3);
                progressBar.setMax(loader.getFileSize());//设置进度条的最大刻度为文件的长度

                try {
                    loader.download(new FileService.DownloadProgressListener() {
                        @Override
                        public void onDownloadSize(int size) {//实时获知文件已经下载的数据长度
                            Message msg = new Message();
                            msg.what = 1;
                            msg.getData().putInt("size", size);
                            Log.i("download", "onDownloadSize: size=" + size);
